...
export async function newPage(options) {
    ...
    if (fs.existsSync(htmlFilePath) === false) {
        // 创建页面
        const { pageTemplate } = await import(yicodeUtils.relativePath(yicodeUtils.fn_firname(import.meta.url), path.resolve(yicodePaths.webpackDir, 'template', 'pageTemplate.js')));

        let htmlFileData = _.template(pageTemplate)(options);
        fs.outputFileSync(htmlFilePath, htmlFileData);

        // 创建页面路由
        const { pageRoute } = await import(yicodeUtils.relativePath(yicodeUtils.fn_firname(import.meta.url), path.resolve(yicodePaths.webpackDir, 'template', 'pageRoute.js')));
