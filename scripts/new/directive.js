...
export async function newDirective(options) {
    ...
    // 创建页面
    let htmlFilePath = path.join(directiveDir, options.fileNames.camelCaseName + '.js');
    if (fs.existsSync(htmlFilePath) === false) {
        const { directiveTemplate } = await import(yicodeUtils.relativePath(yicodeUtils.fn_firname(import.meta.url), path.resolve(webpackDir, 'template', 'directiveTemplate.js')));
