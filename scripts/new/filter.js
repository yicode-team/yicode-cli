...
export async function newFilter(options) {
    ...
    if (fs.existsSync(htmlFilePath) === false) {
        let filterPath = yicodeUtils.getFileProtocolPath(path.resolve(yicodePaths.webpackDir, 'template', 'filterTemplate.js'));

        const { filterTemplate = '' } = await yicodeUtils.importModule(filterPath, {});
